def get_peak_file(wildcards):
    ...
    if wildcards.peak_caller_type == "macs2_narrow":
        bed=join(RESULTSDIR,"peaks","macs2",wildcards.qthresholds, wildcards.treatment_control_list + "." + wildcards.dupstatus + ".narrow.peaks.bed")
    if wildcards.peak_caller_type == "macs2_broad":
        bed=join(RESULTSDIR,"peaks","macs2",wildcards.qthresholds, wildcards.treatment_control_list + "." + wildcards.dupstatus + ".broad.peaks.bed")

    # SEACR OPTIONS
    if wildcards.peak_caller_type =="seacr_norm_stringent":
        bed=join(RESULTSDIR,"peaks","seacr",wildcards.qthresholds, wildcards.treatment_control_list + "." + wildcards.dupstatus + ".norm_stringent.peaks.bed")
    if wildcards.peak_caller_type =="seacr_norm_relaxed":
        bed=join(RESULTSDIR,"peaks","seacr",wildcards.qthresholds, wildcards.treatment_control_list + "." + wildcards.dupstatus + ".norm_relaxed.peaks.bed")
    if wildcards.peak_caller_type =="seacr_non_stringent":
        bed=join(RESULTSDIR,"peaks","seacr",wildcards.qthresholds, wildcards.treatment_control_list + "." + wildcards.dupstatus + ".non_stringent.peaks.bed")
    if wildcards.peak_caller_type =="seacr_non_relaxed":
        bed=join(RESULTSDIR,"peaks","seacr",wildcards.qthresholds, wildcards.treatment_control_list + "." + wildcards.dupstatus + ".non_relaxed.peaks.bed")

    #GOPEAKS OPTIONS
    if wildcards.peak_caller_type =="gopeaks_narrow":
        bed=join(RESULTSDIR,"peaks","gopeaks",wildcards.qthresholds, wildcards.treatment_control_list + "." + wildcards.dupstatus + ".narrow.peaks.bed")
    if wildcards.peak_caller_type =="gopeaks_broad":
        bed=join(RESULTSDIR,"peaks","gopeaks",wildcards.qthresholds, wildcards.treatment_control_list + "." + wildcards.dupstatus + ".broad.peaks.bed")
...
rule rose:
    ...
    output:
        ...
        regular=join(RESULTSDIR,"annotation","{peak_caller}","{qthresholds}","{treatment_control_list}.{dupstatus}.{peak_caller_type}.ROSE.{s_dist}","{treatment_control_list}_AllEnhancers.table.regular.bed"),
        super=join(RESULTSDIR,"annotation","{peak_caller}","{qthresholds}","{treatment_control_list}.{dupstatus}.{peak_caller_type}.ROSE.{s_dist}","{treatment_control_list}_AllStitched.table.super.bed"),
        regular_great=join(RESULTSDIR,"annotation","{peak_caller}","{qthresholds}","{treatment_control_list}.{dupstatus}.{peak_caller_type}.ROSE.{s_dist}","{treatment_control_list}_AllStitched.table.super.GREAT.bed"),
