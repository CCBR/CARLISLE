...
GENERATE_SPIKEIN_PLOT <- function(input_df, spike_type) {
  ...
  p <- ggplot(
    data = spike_df,
    aes(x = sampleid, y = read_count, fill = groupid)
  ) +
...
READ_PEAK_FILE <- function(peak_file_in) {
  ...
  peak_df <- read.csv(peak_file_in, sep = "\t", header = FALSE)[, c(
    "V1",
    "V2",
    "V3"
  )]
...
PLOT_QC_MAIN <- function(function_in, rowid_in) {
  ...
  locusdf_list <- c(
    base::strsplit(locusdf_list_str, ",")[[1]],
    "nearest_tss",
    "nearest_gene"
  )
...
GO_ANALYSIS_MAIN <- function(rowid, peak_enrichment) {
  ...
  if (peak_enrichment == "hybrid") {
    results <- hybridenrich(
      ...
      genesets = geneset_id,
      locusdef = locus_def,
      qc_plots = F, # randomization = 'complete',
      out_name = NULL,
      n_cores = n_cores_in,
      min_geneset_size = 10
    )
    ...
    results <- broadenrich(
      ...
      genesets = geneset_id,
      locusdef = locus_def,
      qc_plots = FALSE, # randomization = 'complete',
      out_name = NULL,
      n_cores = n_cores_in,
      min_geneset_size = 10
    )
    ...
    results <- proxReg(
      READ_PEAK_FILE(peak_df[["peak_bed"]][rowid]),
      reglocation = "tss",
      genome = speciesID,
      genesets = geneset_id,
      out_name = NULL
