...
homer <- lapply(homer, function(x) {
  x[1:rownames(x[which(x$Annotation == "Annotation"), ]) - 1, ]
})
...
homer <- cbind(
  homer,
  colsplit(homer$Sample, "[.]", c("Comparison", "Duplication", "Caller"))
)
homer <- cbind(
  homer,
  colsplit(homer$Comparison, "_vs_", c("Replicate", "Control"))
)
homer <- homer[
  which(homer$Duplication == dupstatus & homer$Caller == peak_mode),
]
homer$`LogP enrichment (+values depleted)` <- as.numeric(
  homer$`LogP enrichment (+values depleted)`
)
...
homer <- dcast(
  ...
  value.var = "LogP enrichment (+values depleted)"
)
