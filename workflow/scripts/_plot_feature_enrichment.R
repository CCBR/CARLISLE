...
homer.files = list.files(path=peaks.dir,pattern="annotation.summary",
                         recursive=TRUE,full.names = TRUE)
homer = lapply(homer.files,function(x) read.table(x,sep='\t',header=TRUE,check.names = FALSE,nrows=14))
names(homer) = gsub("gopeaks/|macs2/|seacr/","",
                    gsub(".annotation.summary","",
                         gsub("/annotation/homer","",
...
hm = Heatmap(homer,
             ...
             col=colorRamp2(breaks=c(-70000,-50,0,50,70000),
