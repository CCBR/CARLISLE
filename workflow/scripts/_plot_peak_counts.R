...
peaks$File <- gsub(
  ".peaks.bed$",
  "",
  gsub("/peak_output", "", gsub("^.*results/peaks/", "", peaks$File))
)
peaks <- cbind(
  peaks,
  colsplit(peaks$File, "/", c("Threshold", "Caller", "Sample"))
)
peaks <- cbind(
  peaks,
  colsplit(peaks$Sample, "[.]", c("Comparison", "Duplication", "Mode"))
)
peaks$Caller <- paste(peaks$Caller, peaks$Mode, sep = "_")
peaks <- peaks[, c("Comparison", "Caller", "Duplication", "Threshold", "Peaks")]
peaks <- cbind(
  peaks,
  colsplit(peaks$Comparison, "_vs_", c("Replicate", "Control"))
)
...
for (threshold in unique(peaks$Threshold)) {
  ...
  peaks_threshold_dup <- dcast(
    peaks_threshold,
    Replicate + Caller ~ Duplication,
    value.var = "Peaks"
  )
  cor <- round(
    as.numeric(unlist(cor.test(
      peaks_threshold_dup$dedup,
      peaks_threshold_dup$no_dedup
    ))["estimate.cor"]),
    2
  )
  peaks.max <- max(
    max(peaks_threshold_dup$dedup),
    max(peaks_threshold_dup$no_dedup)
  )

  png(
    paste("duplication_corr.", threshold, ".png", sep = ""),
    width = 350,
    height = 300
  )
  print(
    ggplot(peaks_threshold_dup, aes(x = log10(no_dedup), y = log10(dedup))) +
      geom_point(aes(color = Caller)) +
      geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
      xlab("log10(Peaks), no deduplication") +
      ylab("log10(Peaks), deduplication") +
      xlim(0, log10(peaks.max) + 0.5) +
      ylim(0, log10(peaks.max) + 0.5) +
      ggtitle(
        paste(
          "Peaks by read duplication, q-value threshold ",
          threshold,
          sep = ""
        ),
  ...
  png(
    paste("peaks_by_caller.", threshold, ".png", sep = ""),
    width = width,
    height = height
  )
  print(
    ggplot(
      peaks_threshold[which(peaks_threshold$Duplication == "dedup"), ],
      aes(x = Sample, y = Peaks)
    ) +
      geom_boxplot() +
      facet_wrap(
        ~Caller,
        nrow = length(unique(peaks_threshold$Caller)),
        scales = "free_y"
      ) +
