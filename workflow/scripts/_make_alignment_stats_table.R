...
parser$add_argument("--yamlDir", type="character", required=TRUE,
                    help="absolute path to location of yamls")
parser$add_argument("--excludeFromName", type="character", required=TRUE, 
                    help="sample info as TSV")
parser$add_argument("--scaleConstant", type="double", default = 1e6, required=FALSE, 
                    help="scaling constant, typically 1e6.[default %(default)s]")
parser$add_argument("--outTable", type="character", required=TRUE,
...
setwd(yaml_dir)
files=list.files(path = yaml_dir,
                 pattern = "*.yaml")
df = data.frame()
for ( f in files ){
