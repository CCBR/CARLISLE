...
parser.add_argument(
    ...
    required=True,
    type=str,
    help="list of peak calls and scaled bedgraph file, tab delimited, group|sampleName|bedfile|bedgraph|scalingFactor|fragmentsBed per line",
)
...
parser.add_argument(
    ...
    required=True,
    type=str,
    help="bedgraph-AUC-based output counts matrix TSV",
)
parser.add_argument(
    "--fragmentscountsmatrix",
    required=True,
    type=str,
    help="fragmentsBed-based output counts matrix TSV",
)
parser.add_argument(
    "--sampleinfo", required=True, type=str, help="output sample info TSV"
)
...
if not os.path.exists(args.tmpdir):
    try:
        ...
        raise OSError(f"{args.tmpdir}: Folder does not exist and cannot be created!") from e
...
for i, row in bedbedgraph.iterrows():
    ...
    counts[samplename] = pandas.DataFrame.from_dict(
        peakCounts, orient="index", columns=[samplename]
    )
...
counts_matrix_df = functools.reduce(
    lambda x, y: x.merge(y, left_index=True, right_index=True), counts.values()
)
counts_matrix_df.to_csv(
    args.countsmatrix, sep="\t", header=True, index=True, index_label="peakID"
)
fcounts_matrix_df = functools.reduce(
    lambda x, y: x.merge(y, left_index=True, right_index=True), fcounts.values()
)
fcounts_matrix_df.to_csv(
    args.fragmentscountsmatrix, sep="\t", header=True, index=True, index_label="peakID"
)
