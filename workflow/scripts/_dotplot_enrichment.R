...
save_empty_plot <- function(
  outfile,
  title = "No GO Enrichment Results",
  subtitle = "Input TSV is empty."
) {
  p_empty <- ggplot() +
...
if (length(args) == 0) {
  stop(
    "Usage: Rscript plot_enrichment_ggplot.R --input in.tsv --output out.png [--top_n 20] [--y_fontsize 8] [--wrap_width 38]"
  )
}
...
get_arg <- function(flag, default = NULL) {
  ...
  if (idx[length(idx)] == length(args)) {
    stop(paste0("Missing value for ", flag))
  }
...
pvalue_col <- pick_col(
  df,
  c("P.value.x", "P.value", "P.value.Hybrid", "pvalue")
)
fdr_col <- pick_col(df, c("FDR.Hybrid", "FDR", "adjPval", "padj", "qvalue"))
geneset_col <- pick_col(
  df,
  c("N.Geneset.Genes", "NumGenes", "GeneCount", "Count")
)
peak_col <- pick_col(
  df,
  c("N.Geneset.Peak.Genes", "NumPeaks", "num.peaks", "Count")
)
ratio_col <- pick_col(df, c("GeneRatio", "gene_ratio"))
desc_col <- pick_col(
  df,
  c("Description", "Term", "Pathway", "Name", "Geneset.ID")
)

missing <- c()
if (is.null(status_col)) {
  missing <- c(missing, "status column")
}
if (is.null(pvalue_col)) {
  missing <- c(missing, "p-value column")
}
if (is.null(geneset_col)) {
  missing <- c(missing, "geneset size column")
}
if (is.null(peak_col) && is.null(ratio_col)) {
  missing <- c(missing, "peak count or GeneRatio column")
}
if (is.null(desc_col)) {
  missing <- c(missing, "description column")
}
if (length(missing) > 0) {
...
df <- df[
  tolower(trimws(as.character(df[[status_col]]))) == "enriched",
  ,
  drop = FALSE
]
if (nrow(df) == 0) {
  save_no_data_and_quit(outfile, "No enriched pathways found.")
}

df[[pvalue_col]] <- suppressWarnings(as.numeric(df[[pvalue_col]]))
df <- df[is.finite(df[[pvalue_col]]) & df[[pvalue_col]] > 0, , drop = FALSE]
if (nrow(df) == 0) {
  save_no_data_and_quit(outfile, "No enriched pathways with valid p-values.")
}
...
if (!is.null(peak_col)) {
  df[[peak_col]] <- suppressWarnings(as.numeric(df[[peak_col]]))
}
if (!is.null(ratio_col)) {
  df[[ratio_col]] <- suppressWarnings(as.numeric(df[[ratio_col]]))
}

df <- df[is.finite(df[[geneset_col]]) & df[[geneset_col]] > 0, , drop = FALSE]
if (nrow(df) == 0) {
  save_no_data_and_quit(outfile, "No rows with positive geneset size.")
}
...
if (nrow(df) == 0) {
  save_no_data_and_quit(outfile, "No plottable rows after filtering.")
}
...
fdr_plot <- ifelse(
  is.na(fdr_plot) & is.finite(p_vals) & p_vals > 0,
  p_vals,
  fdr_plot
)
if (all(!is.finite(fdr_plot))) {
  save_no_data_and_quit(
    outfile,
    "No positive FDR/p-value values available for plotting."
  )
}
