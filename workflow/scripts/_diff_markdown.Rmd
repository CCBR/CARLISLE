...
# list packages
list.of.packages=c("BiocManager","DESeq2","edgeR", "tidyverse","dplyr", "DT","reshape2","pander","plotly", "ggplot2", "ggfortify", "ggrepel", "ELBOW", "yaml", "EnhancedVolcano", "ChIPseeker","stringr","RColorBrewer","HTSFilter", "TxDb.Mmusculus.UCSC.mm10.knownGene", "org.Mm.eg.db", "TxDb.Hsapiens.UCSC.hg19.knownGene","org.Hs.eg.db", "TxDb.Hsapiens.UCSC.hg38.knownGene")
...
# If there are less than 1K but more than 150 then set the nsub value to the n rows
# if there are not then variancestablizing must be used
if (sum( rowMeans( counts(dds, normalized=TRUE)) > 5 ) > 1000){
  rld <- vst(dds)
} else if (sum( rowMeans( counts(dds, normalized=TRUE)) > 5 ) > 150){
