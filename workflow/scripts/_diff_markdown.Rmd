...
# can't replace with the original value since max integer is 2147483647
...
# If there are less than 1K check whether there are enough instances to use nsub value set to the n rows
...
if (sum(rowMeans(counts(dds, normalized = TRUE)) > 5) > 1000) {
  ...
  if (class(t)[1] == "DESeqTransform") {
    ...
    print("VST: Using variance Stabilizing transformation")
...
if (params$species == "mm10") {
  ...
  ## create TXDB object
  message(paste("GTF:", params$gtf))
  txdb <- makeTxDbFromGFF(params$gtf,
    format = c("auto"),
    dataSource = "Downloaded from https://ftp.ensembl.org/pub/rapid-release/species/Homo_sapiens/GCA_009914755.4/ensembl/geneset/2022_07/Homo_sapiens-GCA_009914755.4-2022_07-genes.gtf.gz",
    organism = "human",
    taxonomyId = 9606,
