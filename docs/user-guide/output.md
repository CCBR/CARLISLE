...
- **`rose_input_SuperEnhancers_ENHANCER_TO_GENE.txt`**: enhancer-centric table generated by Younglab `ROSE_geneMapper.py` with gene linkage columns (`OVERLAP_GENES`, `PROXIMAL_GENES`, `CLOSEST_GENE`). Use this file to map each super-enhancer to candidate target genes.
- **`rose_input_SuperEnhancers_GENE_TO_ENHANCER.txt`**: gene-centric view of the same mapping, listing genes and their associated super-enhancer regions.
